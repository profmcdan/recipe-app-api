...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_recipe_partial_update(self):
        """Test partial updating of recipe with patch"""
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_filter_recipes_by_tags(self):
        """Test returning recipes with specif tags"""
        recipe1 = sample_recipe(user=self.user, title='Thai veg')
        recipe2 = sample_recipe(user=self.user, title='Yourgout veg')
        tag1 = sample_tag(user=self.user, name='Vegan')
    ...
    def test_filter_recipe_by_ingredients(self):
        ...
        recipe1 = sample_recipe(user=self.user, title='Thai veg')
        recipe2 = sample_recipe(user=self.user, title='Yourgout veg')
        ingredient1 = sample_ingredient(user=self.user, name='Stick')
        ingredient2 = sample_ingredient(user=self.user, name='Muchroom')
